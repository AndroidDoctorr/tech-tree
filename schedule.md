# Schedule — deadlines, routines, seasons

*Last updated: **Day 1068** · **Cal-Y4 D101** · **Week 153 · ×64 green drying · fire d1069+*

---

## Now @ Day 1068

| Track | Status |
|-------|--------|
| **Hero** | **GREEN-TOUCH-TEST d1069 · KILN-B fire · brick sprint · gap ~59** |
| **Factory** | **×64 green drying · pile 1 ~12.5 kg · stackable ~197** |
| **Forge** | **Lane ~7.8 kg · pile 5 ~81 kg · vault ~68.6 kg** |
| **Apiary** | **HIVE-1 hands-off · pollinator DEFER** |
| **Glass / amphora** | **×3 bottles @ rack L · AMPHORA-4 ✓ fired transport #4 · frit ~120 g** |
| **Beach kit** | **✓ home deploy · chair · towels ×2 · umbrella · swimsuit · COAST sand trip backlog** |
| **WW-YARD** | **STORE-4 N · CHEM-LAB S · WW-HUB-TOWER ✓ · oak floor · whitewash · connector live** |
| **ORE-WASH-SIFT-1** | **✓ · process haul @ HOME** |
| **STOCK-REPLENISH** | **✓ STORAGE-SPRINT CLOSE · overflow queue filed d1027 · horreum post-sow** |
| **Lab wear** | **CL-LAB-TUNIC-1 ✓ @ lime peg** |
| **→ Harvest hard block** | **✓ closed d967 · FEAST-970 ✓ d970 · SUN-CAL-Y3-CLOSE ✓ · EMMER ✓ · P-17 ✓** |

### Season clocks

| Clock | Days | Cal | Notes |
|-------|------|-----|-------|
| **→ ICE haul ideal** | **~998–1023** | **Y4 D30–D55** | **✓ thin haul d1004 @ D37 · next year scout @ D30 · max @ D49–D51** |
| **→ Spring sow window** | **~1034–1052** | **Y4 D67–D85** | **✓ SPRING-SOW-Y4 d1034 · FROST-GATE-Y4 OPEN · scare-only · ~2 d tail** |
| **→ APIARY / swarm window** | **~1057–1087** | **Y4 D90–D120** | **WEEK-151 sprint · kit d1051–1056 · scout d1057+ · veil required** |
| **→ EXPED / cart trips** | **~1047–1247** | **Y4 D80–D280** | **Opens ~10 d @ D70 · dry season band** |
| **→ Autumn harvest** | **~1309–1332** | **Y4 D341–365** | **Hard block** — no casual trips |
| **→ Feast / Yule eve** | **~1332** | **Y4 D365** | **Solstice pin @ C-0 · 46/42** |
| **Yearly windows** | **—** | **All Cal-Y** | **✓ SUN-CAL-YEARLY-WINDOWS-1004 @ C-0 + slate** |

### Priority stack *(reprioritized Day 812)*

| Order | Item | When | Notes |
|-------|------|------|-------|
| **0** | **QUARTZ-SEASON-CLOCK** | **Primary closed @850 · ~45.9 kg sufficient** | **Secondary ~900–930 if tonnage · clarity = method** |
| **1** | **CHAR-WOOD-SPRINT-1** | **✓ d813** | **Lane ~16 kg · pile ~24 kg · vault ~68 kg** |
| **2** | **ORE-WASH-SIFT-1** | **✓ d814** | **Trickle tray @ chem sill · Belen demo PASS** |
| **3** | **QUARTZ-CART-HAUL-1** | **✓ d819** | **~46.1 kg clean FACE-B banked** |
| **4** | **GLASS-GEAR-UPGRADE-1** | **~85% @826** | **Vent last · farm · GLASS-PRACTICE** |
| **5** | **LAB-VENT-FAN-1** | **✓ d836** | **Slip @ HOME while on kelp trip** |
| **6** | **FARM-STRUCT-READ-1** | **✓ d831** | **Early · player override · GREEN** |
| **7** | **GLASS-PRACTICE-1** | **d841+ Week 121** | **Quality ladder · no vessels yet** |
| **5** | **GLASS-PRACTICE-1** | **~d830–834** | **After gear queue · Belen bead/disk** |
| **6** | **TRIP-HAMMER-1** | **Interleave d825+** | **WW-1 proto · see expanded plan** |
| **6** | **HOME-COMFORT / wardrobe** | **Interleave** | **Shoes · socks · tunic · laundry on cure days** |
| **7** | **BEACH / COAST craft** | **Post quartz haul** | **Chair · umbrella · towel — downtime reward** |
| **8** | **STOCK-REPLENISH · STONE** | **When pipeline breathes** | **Brick · dress stone** |
| **9** | **APIARY-1** | **✓ HIVE-1 d1059** | **Queen · skep · pad · care · honey year two+** |

### Loose ends & deferred *(not abandoned — ranked)*

| ID | Status | Notes |
|----|--------|-------|
| **STORE-3 demo** | **Optional** | **RETIRED empty Day 797** — thatch holds until decision |
| **SHELF-IRON-FRAME-v2** | **Deferred** | Post-replenish forge practice · oak v1 ✓ |
| **CRANE-2** | **Brainstorm only** | @ map.md · after replenish · not CRANE-1 relocate |
| **COAST-SCOUT-1** | **Scrubbed Day 750** | Prep banked · **COAST-SHAKEDOWN ✓** · not blocking |
| **LAB-VENT-FAN-1** | **Queued** | Hood chase provisioned Day 703 |
| **STOCK-REPLENISH-1** | **Paused Day 804** | **~49 stackable OK for scout · resume before big build** |
| **TIN-SCOUT-1 / Tarsus** | **Cal-Y4+** | **Not this year** — bridges · coastal path · cart/team upgrades first |
| **HORSE-SCOUT / BOAT-1** | **Horizon** | Optional mouth recon · not blocking tin land route |
| **MADDER-DIG-2 + MADDER-BED** | **✓ d966** | **MADDER-DIG-2 ~144 g dry · crowns ×4 planted · UMB/CHAIR stripe GO · WOAD seed Cal-Y4 spring** |

### APIARY pipeline *(WEEK-151 LIVE Day 1051 · site staked d907)*

| # | ID | Est. | Status @ d1051 | Notes |
|---|-----|------|----------------|-------|
| **0** | **APIARY-PAD-1** | **½ d** | **✓ d1054 AG-AP-01** | **Stand ~60 cm · moat · W-CHAN drip · SKEP-1 mounted** |
| **1** | **REED-SKEP-1** | **1–2 d** | **✓ d1053 ~40 L @ pad** | **Throat ~120 mm · mud lip crown · stand before capture set** |
| **2** | **SMOKER-1 + VEIL-1** | **½–1 d** | **✓ d1055 · practice PASS** | **Fired bowl · reed tube · smolder pack · linen veil · CL-GLOVE-H cuff** |
| **3** | **HIVE-SCOUT-1** | **1 d** | **✓ d1057 HIVE-SITE-GW-01** | **Gorge W colony · crevice 3.6 m · capture @ flight line · not cut-out** |
| **4** | **SWARM-CAPTURE-1** | **1 d** | **✓ d1059** | **Cast ~3.8k · QUEEN-PASS · HIVE-1 @ AG-AP-01** |
| **5** | **HIVE-CARE-1** | **ongoing** | **✓ OPEN d1059** | **Hands-off hold · weekly calm pass · no harvest year one** |

**Rejected apiary sites:** C-0 (H-V3 court) · WW-YARD (smoke) · H-v2 N terrace (masonry traffic).

### Waste / QoL stack *(filed Day 912 · post trip-hammer mass)*

| # | ID | Notes |
|---|-----|-------|
| **0** | **SLAG-HAUL-1** | **✓ d923 first cart ~18 kg @ WF-SB-01 · dish clear · repeat as blooms add** |
| **1** | **DRAIN-D-1** | **✓ d915 · swale · greywater · grease-trap stub · midden tie** |
| **2** | **GREASE-TRAP-1** | **✓ d919 · kitchen → swale · DR-GT-01 · reed baffle live** |
| **3** | **SOIL-PIT-1** | **✓ d925 · DR-SP-01 · ~2.0 m sealed · LATRINE-SEAT unlocked** |
| **4** | **FLUSH-TOILET-1** | **✓ d938 live · FLUSH-RESERVOIR d937 · never Orontes** |
| **5** | **BIDET-1** | **✓ d935 · stem tee · grey → DR-GT-01** |

### Hygiene pack *(filed Day 919 · no-smelt interleave on pit/cure days)*

| # | ID | Iron? | Status |
|---|-----|-------|--------|
| **0** | **SOIL-PIT-1** | No | **✓ d925 close** |
| **1** | **LATRINE-SEAT-1** | No | **✓ d934 live @ DR-LT-01 → DR-SP-01** |
| **2** | **GREASE-TRAP-1** | No | **✓ d919** |
| **3** | **BIDET-1** | No | **✓ d935 live @ DR-LT-01 · stem tee · grey → DR-GT-01** |
| **4** | **SHOWER-3** | No | **✓ d939 closed @ DR-LT-01 · header arm · reed wrap · hygiene epic complete** |
| **5** | **LAUNDRY-ROTATION** | No | **Char-cure afternoons · SOAP-1 · nook CL-TOWEL-2 split ✓ d941** |
| **6** | **HAND-WASH-STATION-1** | No | **✓ d941 @ P-GOAT-1 gate · spigot · soap peg · grey swale** |
| **6b** | **HYGIENE-TOWEL-PEG-1** | No | **✓ d941 @ DR-LT-01 · CL-TOWEL-2** |
| **6c** | **BOOT-WASH-1** | No | **✓ d941 @ T-5 threshold · gravel tray · mud doctrine** |
| **6d** | **NAIL-BRUSH-1** | No | **✓ d941 bone + reed @ hygiene nook** |
| **7** | **CAMP-LADDER-1** | No | **✓ d934 deploy · portable ~3.8 m oak · nails + lash** |
| **8** | **FLUSH-RESERVOIR-1** | No | **✓ d937 · ~35 L @ DR-LT-01 · reed valve · header branch** |
| **9** | **FLUSH-TOILET-1** | Soft | **✓ d938 live · pressure flush · pull cord @ DR-LT-01 → DR-SP-01** |
| **—** | **IRON-SHEARS-1** | **Yes** | **✓ d936 · ~148 g @ WORKBENCH-1 · linen/flax trial PASS** |

### Expedition windows

| Window | Days | Cal-Y3 | Status |
|--------|------|--------|--------|
| **Late spring** | **~780–850** | **~D178–248** | **QUARTZ-SURVEY primary** |
| **Early autumn** | **~900–930** | **~D298–328** | Secondary |
| **Hard block** | **943–967** | D341–365 | Harvest — no casual trips |

---

## How to read dates

| Track | Example | Use for |
|-------|---------|---------|
| **Day N** | **797** | Absolute timeline · never resets |
| **Cal-Yn Dk** | **Cal-Y3 D195** | Deadlines · feast · harvest windows |

**Formula:** `Cal-Y2 D(N−237)` for **238 ≤ N ≤ 602**. **Cal-Y3 D(N−602)** for **N ≥ 603**. **Feast eve** @ **Day 237 + 365×(Y−1)** → Cal-Y2 closed **Day 602**.

**Season phase** keys off **SUN-CAL-1** — see [sun-calendar.md](sun-calendar.md).

**Assistant rule:** Before a hero week, warn if plan crosses a **Hard** deadline in **7–14 days**.

---

## 1 — Daily grind

| Task | Window | Skip if |
|------|--------|---------|
| **Farm scare** | Dawn | Multi-day expedition — not a full week |
| **Pen / holding** | Dawn | — |
| **Weir** | Dawn or dusk | Ice / flood |
| **SUN-MARK + SUN-SPECK** | Clear noon @ C-0 | Cloud — no guess |
| **Hypocaust / hearth** | Evening | Summer minimal |
| **Respawn** | Night | **H-v2 PROPER-BED-1** |

---

## 2 — Routine checks

| Check | Cadence | Last PASS |
|-------|---------|-----------|
| **FARM-READ** | Weekly spring–autumn | **Hard block · P-17 d944 · scare ongoing** |
| **Char lane @ forge** | Weekly | **~2 kg lane · vault ~51 kg** |
| **Wood pile 5** | Weekly | **~6 kg · tag before pit** |
| **AQUEDUCT drift** | Monthly | **Day 571 tune class** |
| **SEED-VAULT labels** | Monthly | **@ v1 core** |
| **STORE-4 zones** | After reorg | **Day 797 tune ✓** |

---

## 3 — Agricultural calendar

### Cal-Y3 — done / fixed

| Event | Day | Status |
|-------|-----|--------|
| **SPRING-SOW-3** | **669** | **✓** |
| **COAST-SHAKEDOWN-1** | **741** | **✓** |
| **CALORIE-BANK top-up** | **748** | **~97% ✓** |
| **CHEM-LAB Stage 5** | **766** | **✓ live** |
| **STORE-4 shell + fit-out** | **768–797** | **✓ · reorg ✓** |

### Cal-Y3 — upcoming

| Window | Est. Day | Event | Priority |
|--------|----------|-------|----------|
| **Now** | **797+** | **STOCK-REPLENISH · LAB-OPS interleave** | **Now** |
| **~780–850** | | **QUARTZ-SURVEY** | Soft |
| **~892–902** | | **DONKEY-HUNT-3 window** | Medium |
| **943–967** | | **Autumn harvest · feast sprint** | **Hard** |

**Field doctrine:** **Scare-only** when **FARM-READ GREEN**. Never hoe over crop lines for factory hero days.

---

## Post-quartz pipeline *(filed Day 804 · after scout)*

**Canon:** **Upgrade Kiln C** — not a new “Kiln D.” Kiln C hot blast ✓; need **insulation + soak window + tuyere tune** for vessel glass.

**Store chips dry** @ STORE-4 or chem cabinet until melt queue opens — survey does **not** wait on upgrade.

### Harvest fit @ Day 804

| Clock | Days from 804 | Verdict |
|-------|---------------|---------|
| **→ Harvest hard block** | **~139 d** (Day 943) | **Pipeline fits** @ ~10–18 hero-days + interleave |
| **Quartz primary window** | **~46 d** left | **Scout now** · process after return OK |

**Risk (Cal-Y3):** **TIN-SCOUT deferred to Cal-Y4+** — no season conflict this year. Full glass pipeline + stone/beach interleave **fits** before Day 943.

### Ordered queue *(your sequence + canon gates)*

| # | ID | Est. | Blocks glass? | Notes |
|---|-----|------|---------------|-------|
| **0** | **QUARTZ-SURVEY** | **✓ d806** | — | **Zone + cart park marked · samples banked** |
| **0b** | **QUARTZ-CART-HAUL-1** | **horizon** | — | **Wagon to CART-PARK spike · vein pinpoint on that trip** |
| **1** | **KILN-C-UPGRADE-1** | **✓ d809** | **Yes** | **Soak hold PASS · glass vessel class** |
| **2** | **POT-LAB-SET-1** | **1–2 d** | Soft | **✓ d811 close** |
| **3** | **LAB-TOOLS-1** | **1 d** | Soft | Bench overflow · staging |
| **4** | **ORE-WASH-SIFT-1** | **✓ d814** | **Yes** | **Aqueduct trickle tray @ chem sill · magnet sweep** |
| **5** | **ORE-CRUSHER-1** *(WW-1 proto)* | **1–2 d** | Soft | Fine crush — **trip hammer interleave** |
| **6** | **LAB-FIRE-SUPPRESS v1** | **½–1 d** | Soft | Sprayer/mist before heavy fire-table work |
| **7** | **LAB-VENT-FAN-1** | **2–3 d** | Soft | Hood chase · WW belt or crank |
| **8** | **GLASS-PRACTICE-1** | **interleave** | — | Local M-10 baseline → Belen chips after **1+4** |
| **9** | **Glass vessels / labware** | **2–4 d** | — | After anneal routine stable |
| **10** | **CHEM-LAB-AIRLOCK-1** | **✓ d998** | — | **Porch @ lab N · tower S/N mats · STORE S queued** |
| **11** | **STORAGE-JAR-SET-1** | **1 d** | — | Reagent overflow |
| **12** | **CL-LAB-WEAR ×2** | **loom interleave** | — | Rotation tunics · optional |

**Parallel (not blocking glass):** **STONE-CUT-PRACTICE** · **STOCK-REPLENISH resume** · **COAST-VACATION craft**

---

## Stone practice queue *(filed Day 804)*

**Goal:** Roman-scale stonework — dressed faces · footings · **stone brick** trials · voussoir/lintel grammar.

| # | ID | Est. | Notes |
|---|-----|------|-------|
| **1** | **STONE-CUT-PRACTICE** | **½ d weekly** | Limestone + FS dress @ pile 4 · chisel + adze + STRAIGHT-EDGE-2 |
| **2** | **STONE-BRICK-TRIAL-1** | **1 d** | Cut soft stone blocks to **brick module** (~220×110×50 mm class) · test mortar bed |
| **3** | **STONE-COURSE-MOCK-1** | **1 d** | Short **dry-stack + lime** course · plumb · square · no hypocaust |
| **4** | **STONE-SILL-TRIAL-1** | **½ d** | Threshold/lintel dress · bridge/footing prep |
| **5** | **STONE-BRIDGE-PREP** | **horizon** | After **STONE-BRIDGE-1** grammar @ km 3 crossing |

**Interleave on:** factory off-days · kiln cure days · post-quartz afternoons.

---

## CHAR-WOOD-SPRINT-1 *(filed Day 810 · before glass hero work)*

**Why now:** **Kiln C upgrade** burned **~5 kg** lane class · **Kiln B** lab fire drew more · lane **~3 kg** · doctrine says **pit before brick+forge dual** · glass soak will want **comfortable lane + vault headroom**.

| Target | Now @810 | Sprint goal |
|--------|----------|-------------|
| **Char lane @ forge** | **~3 kg** | **~10+ kg** |
| **Wood pile 5** | **~18 kg** | **~30+ kg** before dual pit |
| **Vault** | **~51 kg** | Maintain — top-up optional if pit overproduces |

**Suggested rhythm *(2–3 hero days + interleave)*:**

| Block | Work |
|-------|------|
| **811 AM** | **POT-LAB unload · vial batch · close** *(Kiln B pieces — half day max)* |
| **811 PM** | **PIT-CHAR** + scare · or **CART-WOOD-HAUL** if pile tags first |
| **812** | **Wood haul AM · pit PM** *(or dual pit if pile holds)* |
| **813** | **Lane top · vault tag · sprint close read** |

**Interleave while pit cures / char cools:** lab vial labels · wash/sift slate · beach frame strips · loom · wardrobe — **not** Kiln C glass until lane green.

---

## Comfort & wardrobe interleave *(filed Day 810 · reprioritized Day 811)*

**Side project order *(post QUARTZ-CART-HAUL-1 only)*:**

| # | ID | Notes |
|---|-----|-------|
| **1** | **BEACH-CHAIR-1** | Downtime interleave · not before Belen haul |
| **2** | **SHOE-2 / campus pair** | Trail = BOOT-3 |
| **3** | **CL-SOCK · CL-BREECH · spare tunic** | Rotation refresh |
| **—** | **Bedclothes** | **Hold** |

**LAUNDRY-ROTATION:** Wash **tunic · sock · breech · tabard** on **char-cure afternoons** — not hero days. Same-day rule: clean set @ peg before dirty pile grows.

| Also interleave | Est. |
|-----------------|------|
| **CL-LAB-WEAR ×2** | loom · chem rotation |
| **SUN-HAT-2** | beach + farm |
| **LM-3 / STRIPED-TOWEL** | pairs with beach chair trip |
| **Kitchen peg audit** | ¼ d |

---

## GLASS-GEAR-UPGRADE-1 *(filed Day 820 · before GLASS-PRACTICE-1)*

**Doctrine:** **Gear before beads.** Practice melts use **gram batches** — build handling + batch + pour surfaces first.

| # | ID | Est. | Blocks melt? | Notes |
|---|-----|------|--------------|-------|
| **1** | **IRON-GLASS-TONGS-1** | **1 d** | **Yes** | **✓ d820 · ~58 cm reach · cup jaws @ fire-table peg** |
| **2** | **P-POUR-CRUC-1 ×2** | **✓ d822–823** | **Yes** | **PRIMARY + BACKUP · empty soak PASS · glass-ready** |
| **3** | **IRON-POUR-LADLE-1** | **✓ d822** | **Soft** | **~320 g · @ fire-table peg** |
| **4** | **IRON-GLASS-SKIM-1** | **✓ d823** | **Soft** | **~195 g · @ fire-table peg** |
| **5** | **LAB-SCALE-2** | **✓ d824** | **Soft** | **MASS-CHIP set · gram batch slate** |
| **6** | **P-LAB-VIAL-LABEL-1** | **✓ d825** | **G-A–F batch vials · rack A–F refresh** |
| **7** | **GLASS-MARVER-1** | **✓ d825** | **~28×22 cm LF slab @ bench west** |
| **8** | **GLASS-SAND-TABLE-1** | **✓ d826** | **Soft** | **East sill · disk/pane pour @ wash tray** |
| **9** | **LAB-VENT-FAN-1** | **next** | **Soft** | **~4 d belt · farm after · GLASS-PRACTICE** |
| **10** | **KELP-ASH-2** | **½–1 d** | Soft | **Before Day 850** · coast kelp burn or stock repeat · soda resupply |

**Hero total (gear only):** **~9–11 d** → **GLASS-PRACTICE-1 ~Day 830–834 class** if focused.

**Crucible doctrine *(CRUC-GLASS-DOCTRINE-821)*:**

| Risk | Gate |
|------|------|
| **Crack on soak** | **~25% coarse grog body** · **~11–13 mm walls** · **slow dry** · **empty Kiln C soak test before glass** |
| **Metal leach / green tint** | **Partial levigation inner slip only** · **no iron on wet inner face** · **glass-only use** · **never P-δ / copper loads** |
| **Pour lip failure** | **Reinforced rim band** · **v-notch spout** · **stop if chip @ first pour** |
| **Thermal shock** | **Thicker than P-LAB-CRUC** · **peripheral Kiln C preheat** · **tongs grip rim not belly** |

**Mass bootstrap *(MASS-BOOTSTRAP-824)*:**

| Step | Method |
|------|--------|
| **1 Anchor** | **Known mass** — salt batch count on **BALANCE-1** (~27× 1× ≈ 135 g → **1× ≈ 5 g**) |
| **2 Recal coarse** | Re-level **2× · 5×** on workshop beam · re-scratch tags |
| **3 Fine beam** | **LAB-SCALE-2** @ chem — wire pivot · small pans · ~1 g sensitivity |
| **4 Subdivide** | Split **1× → ½× → ¼×** on coarse · verify each on fine beam |
| **5 Cross-check** | Every chip must agree on **both** beams @ 2.5 g · 5 g · 10 g |
| **6 Water check** | **P-vial** mL mark ≈ g class for doubt · re-weigh after humidity swing |
| **7 Log tare** | Empty vial · pan · tray masses on **GLASS-BATCH-SLATE** |

**Interleave (not same day as Kiln C soak):** **TRIP-HAMMER stub** · laundry · **KELP-ASH-2** coast day.

---

## LAB-VENT-FAN-1 · WW belt plan *(expanded Day 820 · materials audit Day 828)*

**Problem:** Chem lab **~140 m** from **WW-1** @ T-1 fork. Hood chase **~20×30 cm** provisioned Day 703 — fan wheel mounts @ east/west cheek pegs.

**Power source today:** **WW-1** rim + **GS-1** flywheel · **GRIND-TAKEOFF ~95%** · modular **slip** grammar from Day 145 blower.

**Belt doctrine *(Day 828 audit)*:** **ROPE-PRIMARY** run **~142 m** — **leather lap wraps** @ **×7** pulleys + splices only. **Not** a continuous **142 m hide strip** (would need **~8 hides** — rejected). Matches **Day 291** grind-belt grammar.

### Material ledger *(audit @ Day 828)*

| Material | Have @ audit | Need | Gap | Resolve |
|----------|--------------|------|-----|---------|
| **ROPE-2** home @ pile 2 | **~59 m** | **~142 m** run | **−83 m** | **+ CRANE reclaim ~46 m** **+ ROPE-TWIST ~37 m** |
| **ROPE-2** @ CRANE-1 | **~46 m** | Reclaim d832 | — | **Crane idle during vent OK** |
| **HIDE-TRIM-SCRAP** @ peg | **~1.8 m²** | **~2.0 m²** wraps | **−~0.2 m²** | **GOAT-HUNT-6 ✓ d829 ~0.97 m²** |
| **GOAT-HUNT-519** @ W-1 | **~0.9 m²** | Reserve | — | **Do not strip for belt** |
| **Beeswax scrap** @ v1 | **~18 g** | **~8 g** lace | OK | **−8 g @ lace day** |
| **TALLOW-1** @ v1 | **~153 g** | **~25 g** dressing | OK | **−12 g tower floor d997** |
| **Oak** @ pile 5 | **WW-WOOD 100%** | posts · pulleys · wheel | OK | **~2.9 kg pulleys d828 · logged per phase** |
| **Cu scrap** | **~12 g** peas | **0** *(pulleys oak-pin)* | OK | **—** |

### Build phases *(time + resources)*

| Phase | ID | Est. | Cum. | Work | Key draws |
|-------|-----|------|------|------|-----------|
| **0** | **LAB-VENT-CRANK-1** *(skip)* | — | — | User deferred | — |
| **1** | **WW-BELT-RUN-SURVEY-1** | **¼ d** | **0.25 d** | **✓ d827** | — |
| **1b** | **WW-BELT-PULLEY-BATCH-1** | **¼ d** | **0.5 d** | **✓ d828** | **~2.9 kg oak · ~0.04 m² trim · oak-pin axles** |
| **2** | **WW-BELT-POST-SET-1** | **1 d** | **1.5 d** | **✓ d827–828** | **×7 oak posts · ×7 pulleys mounted** |
| **2b** | **WW-BELT-STOCK-AUDIT-1** | **¼ d** | **1.75 d** | **✓ d828** | Tallies filed · hunt queued |
| **2c** | **GOAT-HUNT-6** | **1 d** | **2.75 d** | **✓ d829** | **~0.97 m² raw · ~3.4 kg meat** |
| **2d** | **GOAT-HIDE-TAN-829** | **1 d** | **3.75 d** | **✓ d830** | Scrape · brain tan · shade rack |
| **2e** | **ROPE-TWIST-VENT-1** | **1 d** | **4.75 d** | **✓ d832** | **CRANE ~46 m · +~37 m twist · dressing crock · VENT-BELT-ROPE ~142 m** |
| **3** | **WW-BELT-RUN-LAB-1** | **1 d** | **5.75 d** | **✓ d833** | **~142 m lace · hide laps · spin PASS** |
| **4** | **LAB-VENT-FAN-WHEEL-1** | **1 d** | **6.75 d** | **✓ d834** | **Oak shaft · ×8 paddles · bearing @ hood chase** |
| **5** | **LAB-VENT-FAN-LINK-1** | **½ d** | **7.25 d** | **✓ d835** | **#7→wheel · slip pin · low-load drive PASS** |
| **6** | **LAB-VENT-FAN-TUNE-1** | **¼ d** | **~7.5 d** | **✓ d836** | **3× smoke PASS · attended drive · vent close** |
| **7** | **FARM-STRUCT-READ-1** | **1 d** | — | **✓ d831** | **Early player override · GREEN** |
| **8** | **GLASS-PRACTICE-1** | — | — | **d840+** | **Unblocked · after KELP-ASH-2 loop** |

**Calendar @ Day 829:**

| Day | Hero |
|-----|------|
| **830** | Scrape finish · brain tan · shade rack |
| **831** | **FARM-STRUCT-READ ✓ · hide flex · GOAT-SMOKE interleave** |
| **832** | **ROPE-TWIST · CRANE reclaim · belt dressing prep** |
| **833** | Wrap cut · **WW-BELT-RUN-LAB-1** lace + spin |
| **834** | Fan wheel @ hood chase |
| **835** | **Link + slip ✓ · fan driven** |
| **836** | **Tune ✓ · vent close · kelp prep · roll d837** |
| **837–839** | **KELP-ASH-2** *(on trail · **eat wagon bank** · burn @ return)* |
| **840+** | **GLASS-PRACTICE-1** *(vent ✓ · soda leg)* |

**Horizon:** **WW-2 + GS-2** upstream (Day 765 plan) — migrate fan + trip hammer to dedicated mechanical line · keep WW-1 on blast/grind.

**Build order vs glass gear:** **Sand table first** · **vent belt last** (or **crank hedge** if melting before belt done).

**Lab glass zone map *(Day 826)*:**

| Zone | Station |
|------|---------|
| **SW** | Fire-table · tongs · ladle · skim · crucibles |
| **North draft lane** | **MARVER-1** @ hood east cheek · cool · cross-draft |
| **East sill** | ORE-WASH-TRAY · **GLASS-SAND-TABLE-1** · pour disk/pane |
| **North bench** | LAB-SCALE-2 · G-vials · batch prep |

---

## TRIP-HAMMER-1 plan *(filed Day 812 · expanded Day 820 · WW-1 proto)*

**Not blocking glass.** Build on **WW-1 idle / kiln cure / forge-off** afternoons — **never same day as Kiln C glass soak.**

| Phase | ID | Est. | Cumulative | Notes |
|-------|-----|------|------------|-------|
| **0** | **TRIP-HAMMER-1-PLAN** | **✓ d812** | — | Cam · lift · anvil stub · belt takeoff @ WW-1 |
| **1** | **WW-1-TRIP-STUB-1** | **½–1 d** | **~1 d** | Pivot peg · oak lever · **manual test tap** · no cam yet |
| **2** | **WW-1-CAM-1** | **1 d** | **~2 d** | Cam/lift off wheel rim · **belt clearance** recheck · lift **~25–35 cm** class |
| **3** | **TRIP-ANVIL-STUB-1** | **½ d** | **~2.5 d** | Oak block · **iron face plate** · bed level @ WW pad |
| **4** | **TRIP-HAMMER-1 v1** | **1–1½ d** | **~4 d** | **~18–22 kg** iron head · wrist pin · fall rhythm tune |
| **5** | **ORE-CRUSHER-LINK-1** | **1 d** | **~5 d** | **✓ d973 · mortar bowl · grog screen · ~620 g frit PASS** |
| **6** | **Migrate WW-2** | **Horizon** | — | Upstream sluice · **GS-2** · dual-wheel when live |

**Suggested start:** **Day 825+ class** after pour-gear + first crucible fire — interleave **Phase 1 stub** on **Kiln B cure** days.

**Links:** **ORE-CRUSHER-1** proto peg @ WW-YARD · **quartz frit** for glass batch prep.

---

## IRON-ORE-SCOUT-2 plan *(filed Day 892 · player ArcGIS/GE doctrine)*

**Problem:** **M-22-UP lens #2** heavily worked @ **~10.8 kg** max haul — repeat cuts **diminish**. **Tray refilled** but **face depleted**. Modern range has **quarry/mine grammar** in these mountains — **new body likely** within **scout radius**.

**Doctrine — foot vs cart:**

| Mode | When | Why |
|------|------|-----|
| **Wagon → Camp-1 base** | **Day 1 OUT** | Food · water · sacks · tent strip · probe kit · **park covered** |
| **Foot radial scout** | **Days 2–3** | **Pan forks · talus · schist contacts** — **cart does not climb wadi** |
| **Wagon HOME** | **Day 3–4** | **After mark** · optional **modest first cut** if body found early |

**Not:** solo cart same-day sample (**~3 kg** maintenance). **Not:** dragging wagon up **M-22-UP** wadi (d391 grammar).

**Kit:** **IRON-PICK-1 ✓** · **IRON-WEDGE-1** · **MAGNET-PAN-1** · **lodestone needle** · **CU-KNIFE chip** · **yellow spikes ×6** · **sacks ×4** · **BOOT-3** · **BEDROLL-1**

**Search vectors *(3–4 d budget)*:**

| # | Vector | Target |
|---|--------|--------|
| **1** | **M-22 lateral wadis** | **Side forks ±0.5–1.5 km** from pin · **beyond +380 m upstream** |
| **2** | **M-22 talus / black stain** | **Schist contact · strong needle pull** · **not fair pin dilute** |
| **3** | **H-11 bench extension** | **~11 km NE · second face or bench run** · **hematite smelt feed** *(separate from magnetite)* |
| **4** | **Yellow fork deviations** | **D-27 · RS-3 class** · **≤½ d each** if primary vectors dry |

**Success:** **New spike @ body** · **PM-Y1 + black band** · **pan #5 rule** (strong pull = mark, don't strip on scout) · **M-22-UP-2** or **H-11-B** class ID

**After scout:** **Wagon max haul** to new site · **or** return to **H-11 bench** mine grammar (**HAUL-IRON-3** class 30–50 kg)

**Interleave:** **Trip hammer anvil/bloom** waits — **~10.8 kg tray** covers **head + blooms** if scout runs long

---

| Clock | Days | Notes |
|-------|------|-------|
| **→ Primary window close** | **✓ Day 850** | **Skipped return · ~45.9 kg sufficient · clarity = method** |
| **Scout** | **✓ d806** | **CART-PARK + PRIMARY marked** |
| **Critical path** | | **SPRINT → wash/sift → cart haul → glass** |

---

## Beach vacation checklist *(COAST-VACATION-1 · filed Day 804)*

**Trip:** **~20 km W** to **COAST-SITE-1** · **pull post QUARTZ-CART-HAUL** · chair = downtime reward.

**Flex target:** *“Most modern beach day plausible @ ~10,000 BC”* — absurd luxury as flex.

### Must craft *(core kit)*

| ✓ | Item | Build notes | Unlock / have |
|---|------|-------------|---------------|
| ✓ | **FLAX-PULL-3** | Bed B P-07 · fiber + seed same pull · rett load | **✓ d863 · ~9.4 kg @ P-RETT-3 · ~38 g seed · LATE salvage** |
| ✓ | **PANE-CONSISTENCY-WEEK-125** | Pane bank · clarity · template repeat | **CLOSED d875 · 873–874 lantern · 870–872 ~8×10 @ rack G** |
| ✓ | **LANTERN-WEEK-126** | OIL-LANTERN-1 · hang · light | **✓ d882 · v1 portable · v2 slate queued** |
| ✓ | **CAMPUS-MASTER-PLAN-1** | Terraces · paths · drainage · future footprints | **✓ staked d883 · map updated · masonry follows** |
| ✓ | **H-v2-ROOF** | Thatch refresh · tile later | **✓ thatch refresh d883 · TILE deferred** |
| ✓ | **TERRACE-RETAINING-WALL-1** | H-v2 N terrace stake #1 · ~4 m | **✓ d888 · 6 courses + cap · face PASS** |
| ✓ | **TERRACE-RETAINING-WALL-2** | Extension @ stake #2+ · campus plan | **✓ d910 seg #2 · ~8 m total face · PATH-CROWN next** |
| ✓ | **PATH-CROWN-1** | T-5 · T-1 fork @ WW-YARD | **✓ d911 · crown graded · cart PASS** |
| ✓ | **DRAIN-D-1** | Swale · greywater fork | **✓ d915 · GREASE-TRAP ✓ · SOIL-PIT-1 ✓ d925** |
| ☐ | **LATRINE-SEAT-1** | Oak box + chute @ DR-LT-01 | **✓ d934 live @ DR-SP-01** |
| ☐ | **STONE-WALL-SR-S1** | **~28 m Bed D/pen S · SR-S1-01–04 ghost d917 · animal + weed barrier** |
| ☐ | **STONE-WALL-SR-W1** | **~42 m campus W · SR-W1-01–05 ghost d917 · terrace toe grammar** |
| ✓ | **WW-HUB-TOWER-1** | Center gap STORE↔LAB · ~2.75 m clear · heat/light plenum | **✓ interior live · oak · tallow · whitewash · connector on oak** |
| ☐ | **STONE-HAUL-2** | T-2 creek below BRIDGE-1 · FS-class blocks | **✓ d982–983 · ~93 kg total · TOWER-STONE-BANK ✓ · gap closed** |
| ☐ | **LINEN-WING-1** | Dry indoor · hackle/spin/loom | **E stub slate · after tower shell** |
| ☐ | **LM-3-SAIL-LOOM-1** | Wide beam · sail grammar · LOOM-WING | **Slate d888 · after BOAT-1 + flax m² · Cal-Y4+ class** |
| ☐ | **LINEN-WORK** | Repair draw · named weave only | **~234 m fine hold d888 · LM-2 sufficient** |
| ✓ | **FLAX-HACKLE-3** | Scutch s2 · hackle | **✓ d885 · ~275 g line @ FIBER** |
| ✓ | **FLAX-SPIN-PRETT3-1** | ~275 g line → thread bank | **✓ d886 WHEEL-SPIN-10 · ~242 m fine** |
| ✓ | **WHEEL-SPIN-10** | Full bank wheel pull | **✓ d886 · line depleted** |
| ✓ | **WW-1-CAM-1** | Rim cam · lift · fall rhythm | **✓ d890 · ~30 cm · slow-spin PASS** |
| ✓ | **WW-1-TRIP-STUB-1** | Pivot · lever · cam | **✓ d889–890 · powered fall live** |
| ☐ | **TRIP-HAMMER-1** | Head weld · mount · tune · crusher link | **✓ d973 · 100% · ORE-CRUSHER-LINK-1 ✓ · WW-2 migrate horizon** |
| ✓ | **M-22-REPEAT-HAUL-2** | 2-day wagon max @ M-22-UP | **✓ d891–892 · ~10.8 kg · lens heavily worked** |
| ✓ | **HAUL-IRON-5** | **d930–933 · split trip · modest H-11 + M-22 scout · 4/5 d | **✓ d933 · ~56.4 kg H-11 @ pile 4** |
| ☐ | **IRON-ORE-SCOUT-2** | 3–4 d · Camp-1 base · foot radial · new body mark | **Vectors 1–2 ✓ d931 · M-22-TALUS-S1 + M-22-LAT-W750 · full radial deferred** |
| ☐ | **CAMPUS-SOIL-HAUL-1** | Fertile loam · decorative beds · trees/shrubs | **Slate d889 · convenient haul · low maintenance** |
| ☐ | **LANTERN-HANG-CHAIN-v2** | Iron link chain · replace cord bail | **Queued when forge arms agree** |
| ☐ | **LANTERN-FRAME-IRON-v2** | Wrought iron assembly frame | **Queued · v1 oak travels now** |
| ☐ | **LANTERN-BATCH** | More v1 class @ pane form + 870 recipe | **Queued · carry-one model proven** |
| ✓ | **FLAX-BREAK-3** | Break · scutch | **✓ d878 · ~520 g @ W-1 · hackle queued** |
| ✓ | **LANTERN-PANE-FRAME-1** | Sand-table ~6×8 pour mold | **✓ d879 · form-fit PASS · GP-876 pour d880** |
| ☐ | **BOTTLE-WEEK-124** | Iron neck/seat *(paused)* | **GP-868 @ rack G · resume optional** |
| ☐ | **BEACH-UMBRELLA-1** | Split-oak frame · linen canopy · sand pegs ×4 | **✓ d975 · home deploy PASS · COAST sand trip backlog** |
| ☐ | **BEACH-TOWEL-1** | **LM-3 wide strip** or felted wool · **≥150×80 cm** class | **✓ d854 · ~156×80 cm · longwise madder stripe** |
| ☐ | **BEACH-TOWEL-2** | **Backup · plain natural** | **✓ d969 · ~156×80 cm @ v1 FIBER peg** |
| ☐ | **STRIPED-TOWEL-DYE-1** | Clay/wax **dye trough** · dowel dip · madder/grape/iron bands | **✓ d854 accordion bind grammar** |
| ☐ | **SWIMSUIT-1** | Minimal **linen brief** or short tunic | **✓ d974 · ~38×28 cm natural brief @ beach peg** |
| ☐ | **BEACH-MAT-1** | Reed **+** hide backing roll · sand barrier | Reed ✓ |
| ☐ | **BEACH-CHAIR-1** | Low **oak + linen sling** · folding peg grammar | **✓ d969 · teal stripe sling · 100%** |
| ☐ | **SUN-HAT-2** | **CL-HAT** upgrade · wider brim · chin cord | CL-HAT ✓ |
| ☐ | **PICNIC-KIT-1** | Flatbread · salt · fat · jerky · **fig/honey** · **×2 amphora** water | Kitchen ✓ |
| ☐ | **SANDWICH-BOX-1** | Waxed **P-η** dish + lid or birch bark wrap | Pottery ✓ |
| ☐ | **COOL-DRINK-1** | Amphora + **evap wrap** (wet linen) · parched + vinegar splash optional | GRAPE-VINEGAR ✓ |

### Modern-amenity stretch *(reasonable flex)*

| ✓ | Item | Build notes | Era-honest “modern” |
|---|------|-------------|---------------------|
| ☐ | **BEACH-WAGON-1** | **CART-1** kit load plan · umbrella + mat + picnic lashed | You have cart ✓ |
| ☐ | **SHADE-CABANA-1** | **TENT-1** + **umbrella** + side mat walls · 3-sided breeze block | Cabana vibe |
| ☐ | **SNORKEL-1** | **Hollow reed + clay mouth fit** · *face-in-water novelty* | Snorkel silhouette |
| ☐ | **SAND-SIEVE-1** | Willow hoop + linen mesh · **shell hunt** | Sand toy |
| ☐ | **BEACH-SHOVEL-1** | **CU or iron** blade · short oak handle | Sand toy |
| ☐ | **LOUNGE-PILLOW-1** | **Tow batting** roll · linen case · **PROPER-BED** grammar mini | Headrest flex |
| ☐ | **BEACH-BALL-1** | **Bladder** or stitched sphere · airtight seal class | *Extreme stretch* — sphere + seal hard @ era |
| ☐ | **BEACH-RADIO-1** | *Joke tier* — **MUS-1 lyre** @ dune · “portable entertainment” | Morale flex |
| ☐ | **SUNSCREEN-1** | *Stretch* — **lime + olive oil + clay** paste trial · hat + shade primary | Honest: shade not SPF |
| ☐ | **BEACH-BOOK-1** | **Blank clay tablet** or knotted cord journal · nap ledger | “Reading” flex |
| ☐ | **WINE-SKIN-1** | **Goat bladder** or small amphora · **vinegar/wine** class | Ferment ✓ |

### Absurd stretch *(post glass + winter ice)*

| ✓ | Item | Build notes | Era-honest “modern” |
|---|------|-------------|---------------------|
| ☐ | **BEACH-ICED-DRINK-1** | **Glass vessel** + **ICE-HARVEST** winter bank · **insulated cart pack** (hide/reed/sawdust tight) · cold **parched/vinegar/wine** class @ COAST | *Mosquito Coast* flex — ice survives cart run if packed right |
| ☐ | **ICE-HARVEST-1** | **Cal-Y3 winter** — cut/store ice · **COOL-CELLAR** or cart ice box · tight pack doctrine | Prereq for iced drink |
| ☐ | **GLASS-CUP-1** | **Glass vessel** after **GLASS-PRACTICE-1** — honest cup rim | Prereq for iced drink |

**Deps chain:** **QUARTZ haul → glass practice → GLASS-CUP → ICE-HARVEST (winter) → cart pack test → beach vacation w/ iced drink**

### Trip logistics *(already banked)*

| ✓ | Item | Status |
|---|------|--------|
| ☑ | **COVERED-WAGON-1** + team | ✓ |
| ☑ | **COAST-TRAIL-1** + beacon | ✓ |
| ☑ | **TRAIL-CACHE** + shakedown doctrine | ✓ |
| ☑ | **Calorie bank ~97%** | ✓ |
| ☐ | **Pack checklist run** | Day-before vacation |

**Pull triggers:** burnout · **glass vessel ✓** · bronze pour · feast prep · *you want absurdity*.

---

## STOCK-REPLENISH-2 *(filed Day 1027 · post-sprint · pre/post sow interleave)*

**Doctrine:** **STORE-4 north face ↔ WW-HUB-TOWER corridor** — labeled vertical stacks · no floor piles · dry under tile eaves.

**Horreum:** **H-V3-NORTH-HORREUM-1 chamber A ✓ LIVE + stocked d1064 · KILN-A-LIME ✓ · full ghost B/C horizon**.

| # | ID | Est. | Gate | Notes |
|---|-----|------|------|-------|
| **1** | **AMPHORA-4** | **✓ d1033 fired** | **Transport #4 · M-08 pitch optional d1034+** |
| **2** | **IRON-NAILS-BATCH** | **✓ d1028 · d1031 · d1061 · d1062** | **×72 tray · fit-out comfortable · optional ×1 for full audit** |
| **3** | **WOOD-HAUL** | **✓ d1029** | **—** | **~34 kg @ pile 5 · ~6 kg crate billets staged** |
| **4** | **WOOD-CRATE-2+** | **~1 d each** | **Nails** | **✓ WOOD-CRATE-2 d1030 · WOOD-CRATE-3 d1036 · ×0 nails · batch before #4** |
| **5** | **GLASS-BOTTLE-3** | **✓ d1033 CLOSE** | **—** | **×3 @ rack L · frit ~120 g remain** |
| **—** | **H-V3-NORTH-HORREUM-1** | **A stocked** | **d1064+** | **Chamber A ✓ stocked · stock #2 · B/C future · lime runway improved** |
| **—** | **APIARY-1** | **LIVE** | **d1051+** | **WEEK-151 sprint · scout @ d1057 · window D90–D120** |

---

## 4 — Stock maintenance *(Day 813)*

| Stock | Target | Now | Action |
|-------|--------|-----|--------|
| **Stackable brick** | Comfortable shell reserve | **~49** *(paused)* | Resume after haul season |
| **Green brick @ Kiln B** | Factory queue | **~141** | Hoard preserved |
| **Wood pile 5** | **~30+ kg** before pit dual | **~24 kg GREEN** | **OK post-sprint** |
| **Char lane @ forge** | **~10+ kg** class | **~16 kg** | **✓ CHAR-SPRINT close** |
| **Char vault + lane** | **~50+ kg** combined | **~68 vault + ~16 kg lane** | **✓ GREEN** |
| **Iron nails** | **20+ class** | **×20** | **✓ IRON-NAILS-BATCH-803** |
| **Calorie bank** | **~35 kg class** | **~97% @ kitchen/cellar** | Maintain not sprint |
| **H-11 ore** | Smelt runway | **See inventory** | Maintenance blooms |

---

## Related

- [README.md](README.md) — repo map
- [inventory.md](inventory.md) — stock truth
- [map.md](map.md) — geography · campus plan
- [advancements.md](advancements.md) — recent unlocks
- [ladder.md](ladder.md) — achievement queue
- [journal/years/year-002.md](journal/years/year-002.md) — **Cal-Y2 summary**
- [year-002-plan.md](year-002-plan.md) — archived Y2 strategy pointer
- [chem-lab-plan.md](chem-lab-plan.md) — lab stages 6–8
- [sun-calendar.md](sun-calendar.md) — SUN marks · feast anchor

*Update **Last updated** when journal days or reference docs change.*
