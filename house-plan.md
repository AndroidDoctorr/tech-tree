# Domus H-V3 — courtyard compound *(villa · single storey)*

*Revised **Day 640+** — supersedes T-plan / two-storey sketch*  
**Active plan:** **compluvium / impluvium** around **C-0** · **one floor** · **zoned hypocaust**  
**Naming:** Latin labels below — e.g. new **culina** (villa south range) vs **culina vetus** ([H-v2-KITCHEN](map.md) wing · operational until domus live)  
**Superseded:** [images/house-diagram-labeled.png](images/house-diagram-labeled.png) (2F T-plan · filed for reference) · [images/hut-plans.png](images/hut-plans.png) (H-v2 annex sketch)  
**Aligns with:** [materials-roadmap.md](materials-roadmap.md) § courtyard compound (Day 66)  
**Stock / tech:** [inventory.md](inventory.md) · [ladder.md](ladder.md) · [map.md](map.md)

---

## Summary

**Domus H-V3** is a **single-storey courtyard villa** wrapped around **C-0** (~12 m S of H-v2). Ranges face the **atrium** (open court · impluvium drain); **H-v2** becomes the **horreum** (north store range) until obsolete, then **demolish → plaza hardscape**.

**Not in the domus:** **officina** · **telarium** (loom) · **incudinarium** (forge) — those stay **W-1 / MILLHOUSE-3 / FORGE-D** @ WW-YARD ([map.md](map.md)).

**Structural grammar:** **stone socle** + **single-wythe brick mures** (or rubble-lime core in bath wing) · **stone colonnade** on courtyard faces · **zoned hypocaust** (bath block only) · **slab + tile** elsewhere · **shed tile roofs** per range.

**Build doctrine:** **four staged ranges** · **full footing outline @ Stage 0** · **impluvium drain first** · temp **thatch closure** only on range ends until colonnade completes.

### Partial domus *(live while building)*

**Yes** — you do **not** need the full compound before any range is useful.

| Rule | Meaning |
|------|---------|
| **One range = one livable wing** | Roof tile + **mures** closed on that range only · **PANEL-THATCH** on open court faces |
| **H-v2 until cubicula** | **culina vetus** + bed stay @ H-v2 until **west range** passes drip test — then respawn moves |
| **Half compound** | e.g. **south culina live** while **west/east** are footing-only · **atrium** drain can work before all ranges |
| **Parallel clocks** | **Brick factory** · **stone dress** · **domus stages** — interleave; factory never blocks on full hoard |
| **Factory ramp** | **2nd brick mold** · Kiln A/B **stagger** · pit char — raises press rate without waiting on domus |

**Not:** half a roof on one range · hypocaust east before **thermae** shell closed · moving respawn before **tile drip PASS**.

---

## Site · orientation

| Item | Plan |
|------|------|
| **Center** | **C-0** · **SUN-CAL-1** · courtyard axis ([inventory.md](inventory.md)) |
| **North range** | **H-v2** — respawn → **storage / cubiculum annex** when compound live |
| **Slope** | Terrace **falls S** toward workshop lane (Day 93) — **impluvium + all ranges drain to D-1**, never into H-v2 hypocaust |
| **Workshop** | **W-1** @ 80 m downslope — **LM-3 · LOOM-WING** there, not in villa |
| **Forge / heat** | **FORGE-D** @ WW-YARD — not a second villa furnace |

```
              [ horreum — H-v2 · store / cubiculum annex ]
                        |
            (optional vestibulum / covered passage)
                        |
    ┌───────────────────┼───────────────────┐
    │  W · cubicula     │     E · thermae   │
    │  (dormio · vestis)│   (caldarium ·    │
    │                   │    lavatorium)    │
    ├───────────────────┴───────────────────┤
    │         C-0 · atrium / impluvium      │
    │         compluvium · cisterna · sol    │
    ├───────────────────────────────────────┤
    │  S · culina · penus · porticus        │
    └───────────────────────────────────────┘
              ↓ ad D-1 / flumen

    Officina @ W-1 · incudinarium @ FORGE-D (extra campus)
```

---

## Lexicon *(short)*

| Latin | Meaning here |
|-------|----------------|
| **domus** | Villa compound (H-V3) |
| **culina** | **New** kitchen range — **not** culina vetus @ H-v2 |
| **culina vetus** | Existing H-v2-KITCHEN wing — keep until domus culina live |
| **penus** | Pantry / dry stores (in culina range) |
| **cubicula** | Sleep cells |
| **horreum** | Granary / dry store (H-v2 body) |
| **thermae** *(wing)* | Bath block (east) |
| **caldarium** | Hot bath room |
| **lavatorium** | Wash basin |
| **latrina** | Soil toilet |
| **porticus** | Roofed porch · wagon |
| **atrium / impluvium** | Open court · rain drain @ C-0 |
| **compluvium** | Roof opening above impluvium *(optional pool)* |
| **officina** | Workshop @ W-1 |
| **telarium** | Loom bay · LOOM-WING |
| **incudinarium** | Forge district @ FORGE-D |
| **mures / parietes** | Exterior / interior walls |

---

## Room program *(single floor)*

| Range | Label | Rooms | Notes |
|-------|-------|-------|-------|
| **West** | **cubicula** | **2 cells** · **vestiarium** · **loculus** | **Respawn** here when west range roofed · **PROPER-BED-1** |
| **East** | **thermae** | **caldarium** · **latrina** · **lavatorium** · linen · **furnus** | **Only hypocaust** zone · **one flue** |
| **South** | **culina + porticus** | **culina** · **penus** · **porticus** | **AQUEDUCT-1** · oven flue ties east stack |
| **North** | **horreum** | **H-v2** + ante | Amphora · **cella** (COOL-CELLAR-2) beside domus when dug |
| **Court** | **atrium** | **C-0** · **impluvium** · **cisterna** | Rain → barrel → overflow **D-1** |

**Open courtyard rule:** kiln smoke · char dust · wagon turn — **keep court smoke-friendly** ([materials-roadmap.md](materials-roadmap.md)).

---

## Column grid *(~1 m module)*

**Spacing:** colonnade + interior pillars **~1.0 m OC** · stone face **~40–50 cm**.

| Range | Grid *(cols × rows along range)* | Clear interior class |
|-------|----------------------------------|----------------------|
| **West cubicula** | **3 × 4** | **~2.0 × 3.0 m** |
| **East bath** | **3 × 4** | **~2.0 × 3.0 m** |
| **South kitchen** | **4 × 3** | **~3.0 × 2.0 m** |
| **Porticus** | **4 × 1** | **~3.0 × 0.8 m** covered strip |
| **Courtyard** | **~4 × 4 m** clear | Plus colonnade ring |

**Exterior compound (wall to wall):** **~10–11 m** square class — stake from **SUN-CAL-1** anchor.

---

## Walls — **drop double-layer cavity mures**

### Verdict

**Double brick wythes (MURE-2) are dropped** for this plan.

| Why double wall made sense (old T-plan) | Why it does not fit courtyard villa |
|----------------------------------------|-------------------------------------|
| Cavity for **future wire / warm air** | Services run in **parietes** + **colonade service wall** — courtyard face is **open** |
| Two-storey **tie** across cavity | **Single storey** — bond beam at eaves is enough |
| Modern insulation gap | Mediterranean mass wall + **lime plaster** — Roman **thermodynamic mass**, not cavity R-value |

### Replacement wall types

| Type | ID | Use | Build |
|------|-----|-----|-------|
| **Exterior mure** | **MURE-R1** | **Closed** faces (north onto H-v2 · outer compound walls) | **Stone socle 80–120 cm** · **single B-series wythe ~½ brick (≈11 cm)** · **lime plaster** ext + int · oak **lintels** only |
| **Bath mass wall** | **MURE-R2** | East range · heat-facing | **MURE-R1** + optional **rubble-lime core** (grog + lime fill between stone skins) when stock allows — **opus caementicium** class |
| **Courtyard face** | **COLONNADE-1** | **No mure** — open to court | **Stone pillar** · oak **beam** · tile lean roof · rear **PAR-SERVICE** partition |
| **Interior paries** | **PAR-1** | Room divides | **Half brick** or **wattle + lime daub** · **lime plaster** · **chase pockets** for pipes · **OUTLET-BRICK** plugs |
| **Service spine** | **PAR-SERVICE** | Behind colonnade | **Single thick partition** (~½ m) · aqueduct · grey drains · future wire — **inspectable** from court side |
| **Temp closure** | **PANEL-THATCH-1** | Range end until next stage | Removable · **≤2 winters** then swap **lime-daub panel** |

**Electric / wire horizon:** vertical chases in **PAR-SERVICE** and bath paries — **not** in exterior mure.

---

## Floors · hypocaust *(zoned)*

| Zone | Floor stack |
|------|-------------|
| **East bath block** | Footing → gravel weep → **hypocaust void** → suspensura / tile · **grate zones** |
| **West cubicula** | Footing → gravel → **lime slab + tile** · rugs · **no void** *(optional small hypocaust tap from east — defer)* |
| **South culina** | Footing → gravel → **lime slab + tile** · **durable near focus** (hearth) |
| **Porticus** | **Stone flags** or **packed gravel + tile** — splash-resistant |
| **Courtyard** | **C-0** stone pad · **impluvium sump** · slope to drain | **DOMUS-ATRIUM-PAD-1 ✓ d1257 — ~3.6 m Ø flag ring @ compluvium margin** |

**Heat:** **one furnus** in **thermae** (east) · hypocaust **east only** · **culina** oven shares flue via **isolated flue liner**.

---

## Interior textile palette *(DOMUS-TEXTILE-PALETTE · filed Day 1255)*

*Room-color grammar for curtains, rugs, and soft goods — intuitive zoning like school-folder colors. Pigment classes tie to live dye cards ([inventory.md](inventory.md) · **DYE-RECIPE-BURGUNDY-1** · **WOAD-VAT-1** · **GRAPE-PURPLE-542** · verdigris scrape class).*

| Zone | Color family | Grammar | Status |
|------|--------------|---------|--------|
| **Cubicula** *(west + east sleep cells)* | **Red / burgundy** | Madder + iron kiss · curtains + bedside rugs | **West rug ✓ d1119 · east rug ✓ d1255 · curtains horizon** |
| **Culina** *(south kitchen)* | **Forest green** | Woad vat + onion overbath | **Curtain ✓ d1254 · rug ✓ d1256** |
| **Thermae** *(east bath block)* | **Teal / verdigris** | Verdigris scrape · copper patina class · towels/rugs/soft goods | **Horizon** |
| **Everywhere else** *(porticus · penus · atrium margins · PAR service soft goods)* | **Deep mauve / tyrian-class purple** | Grape skin double-dip + iron kiss · chase rich fugitive purple — honest camp, not murex | **Horizon · GRAPE-PURPLE-542 grammar** |

**Rug coverage target:** **≥1 bedside or floor rug per major room** — cubicula ×2 · culina · thermae · porticus/penus class *(atrium = hardscape pad, not rug)*.

**Note:** User filed **teal @ thermae** — if **east cubiculum** was meant instead of bath wing, swap that row only; west cell stays burgundy.

---

## Roof

| Range | Form |
|-------|------|
| **Each range** | **Single shed or gable** · **tile** · **M-08** eave pitch |
| **Porticus** | **Lean-to** off south range · **open wagon side** |
| **Colonnade** | **Continuous eave** around court — **valley** at range corners · **crane-lift** heavy tile pallets (**CRANE-1** ✓) |
| **Bond beam** | **Oak ring** at wall top — **not** needed mid-height (no 2F) |

---

## Plumbing · drainage

### Supply

| Source | Route |
|--------|-------|
| **AQUEDUCT-1** | Header branch → **culina** · **thermae** · **HOT-WATER** class |
| **Impluvium** | Court rain → **cistern** (BARREL-SET) → kitchen/bath makeup |

### Waste *(simplified vs old mega-culvert)*

| Stream | Route |
|--------|-------|
| **Storm / impluvium** | **C-0 sump** → **D-1** → river extension (existing grammar) |
| **Greywater** | **culina** + **lavatorium** → **grease trap** → **planted swale** downslope *(or atrium margin)* |
| **Blackwater** | **latrina** → **SOIL-PIT-1** @ camp margin (~MIDDEN-FAR) · lime-sealed · **not** under atrium |
| **Orontes culvert** | **Horizon / prestige** — only if SOIL-PIT fails or compound scales |

**Flood:** **sluice** on any river outfall you do build · **never** tie blackwater to hypocaust void.

---

## Build stages *(four ranges)*

### Stage 0 — Courtyard stake *(before any hero range)*

- [x] **H-V3-STAKE-1** — compound strings from **C-0** · **fall to D-1** · crane arc *(✓ d1099)*
- [x] **H-V3-FOOTING-OUTLINE-1** — **full square** trench + gravel pad — all ranges same elevation *(✓ d1104)*
- [x] **IMPLUVIUM-DRAIN-1** — **C-0** sump · tile drain · tie **D-1** *(✓ d1105 · pour PASS)*
- [ ] Brick factory — **250+** before first domus mure · **600+** before officina brick cheeks · **1,000+** campus class before thermae hypocaust

### Stage 1 — South range *(culina · penus · porticus)*

**Scope:** **culina** · **penus** · **porticus** · **PAR-SERVICE** stub · aqueduct branch · temp **west + east** court closures. *(culina vetus @ H-v2 stays primary until this passes drip test.)*

| Deliverable | ID |
|-------------|-----|
| South footing + slab floor | **H-V3-SOUTH-FOOT-1** |
| Kitchen + pantry + porticus | **H-V3-CULINA-1** |
| Impluvium drain live | **IMPLUVIUM-DRAIN-1** ✓ |
| Grease trap | **GREASE-TRAP-1** |

#### Culina fit-out *(after shell drip PASS · H-v2 grammar)*

| System | ID | Notes |
|--------|-----|-------|
| **Tile floor** | **CULINA-SOUTH-FLOOR-1** | **✓ d1233** — **Layout E culina bay d1144 · penus/porticus TR ×26 d1233 · boot PASS** |
| **Floor layout** | **CULINA-SOUTH-FLOOR-LAYOUT-1139** | **Layout C** checker **or Layout E** octagon-corner *(light field + iron-slip dark ~5 cm inserts @ 45°)* · **2×2 trial before mass press** |
| **Window @ sink** | **CULINA-SOUTH-WINDOW-1** | **✓ d1148** — **N cheek ~45×40 cm · oak shutter · cross-draft PASS · glass defer** |
| **Sink / basin** | **CULINA-SOUTH-SINK-1** | **✓ d1147** — **P-κ @ N counter · drain → GREASE-TRAP-1 · under window ✓** |
| **Cold pressurized** | **CULINA-SOUTH-WATER-COLD-1** | **✓ d1147** — **AQUEDUCT-SOUTH-BRANCH · ~16 s fill · KITCHEN-FAUCET-1 class** |
| **Hot water** | **CULINA-SOUTH-WATER-HOT-1** | **✓ d1150** — **HOT-WATER-1 draw → south stub · reed wrap · twin tap @ basin · mix PASS** |
| **Hearth** | **CULINA-SOUTH-HEARTH-1** | **✓ d1146** — **slab · throat · flue stack · live smoke PASS** |
| **Flue stack / chimney** | **CULINA-SOUTH-FLUE-STACK-1** | **✓ d1146** — **Brick N cheek through bond beam + TR roof · clay cap · **isolated liner** *(east thermae tie horizon)* · *same deliverable as KITCHEN-FLUE-CHEEK H-v2 grammar — named @ d1146, not retconned* |
| **Cauldron hang** | **CULINA-SOUTH-HEARTH-HOOK-1** | **✓ d1149** — **Iron bar @ bond beam · trammel S-hook · dry ~4 kg PASS** |
| **Vent / draft** | **CULINA-SOUTH-VENT-1** | **✓ d1149** — **Hood lip · reed cheeks · louvers @ window · steam PASS** · **≠ flue stack** |

**Build order:** **drip PASS** ✓ · **floor lay** ✓ · **hearth + flue stack + smoke** ✓ · **sink + cold** ✓ · **window** ✓ · **hook + vent** ✓ · **hot water** ✓ · **H-v2 closeout** ✓.

**Respawn:** stays **H-v2**.

### Stage 2 — West range *(cubicula)*

**Scope:** Sleep cells · wardrobe · tile roof · colonnade posts on court side.

| Deliverable | ID |
|-------------|-----|
| West footing + tile floors | **H-V3-WEST-FOOT-1** |
| Cubicula ×2 + **PROPER-BED** deploy | **H-V3-CUBICULA-1** |
| Colonnade W face | **COLONNADE-W-1** |

**Respawn:** moves to **west cubiculum** after **roof tile drip test PASS**.

### Stage 3 — East range *(bath + hypocaust)*

**Scope:** Bath block · **zoned hypocaust** · **one furnace** · flue stack · latrine to **SOIL-PIT-1**.

| Deliverable | ID |
|-------------|-----|
| East footing + hypocaust | **H-V3-EAST-HYPO-1** |
| Bath · tub · basin · latrine · flush | **H-V3-BATH-1** | **✓ d1252 — tub · basin · hot/cold · latrine · **H-V3-EAST-FLUSH-TOILET-1** live**
| Furnace + chimney | **H-V3-FURNACE-1** |
| Soil pit @ margin | **SOIL-PIT-1** |

### Stage 4 — Close compound *(colonnade ring + north tie)*

**Scope:** Colonnade S+E+W complete · **north passage** to H-v2 (covered or open) · lime plaster pass · remove temp panels.

| Deliverable | ID |
|-------------|-----|
| Colonnade complete | **COLONNADE-RING-1** | **✓ 100% d1246** — **N+S+E+W court beamed+leaned ✓ · stub half-col tied ✓ · H-V3-NORTH-TIE open passage ✓ · SR cover defer · SC-NP-07 ghost optional**
| North tie / horreum grammar | **H-V3-NORTH-TIE-1** |
| Interior plaster | **H-V3-PLASTER-1** | **✓ d1253** — **COMPOUND-PLASTER-1 closed · temp panels off · court open · compound ~99%**
| H-v2 → **store mode** formalized | **H-V2-STORE-1** | **✓ d1254 — horreum A/B/C LIVE · domus north range · doctrine @ kitchen + C-0**

---

## Production · storage off-site *(officina campus)*

| Function | Latin / ID | Location |
|----------|------------|----------|
| **Forge · iron** | **incudinarium** · FORGE-D | WW-YARD |
| **Loom · textiles** | **telarium** · LOOM-WING-1 | W-1 · LM-3 |
| **Bench · woodwork** | **officina** | WORKBENCH-1 @ W-1 |
| **Water · power** | **molendinum** *(millhouse)* | MILLHOUSE-3 @ WW-YARD |
| **Kilns · brick** | **fornax** *(factory)* | Kiln terrace @ v1 — smoke **downwind** of atrium |

---

## Adjacent projects

| Project | Relation |
|---------|----------|
| **COOL-CELLAR-2** | Beside **west or north** range — not under impluvium |
| **FREEZER-VAULT-1** | Sub-chamber in cellar · ice + aqueduct draft |
| **H-v2 demo** | When redundant · **court paving** extension |

---

## Future addons *(horizon)*

| Addon | Notes |
|-------|-------|
| **Wire / EM** | **PAR-SERVICE** chases · **OUTLET-BRICK** |
| **Wagon bay** | Porticus deepen · stone wheel stops |
| **Peristyle shade** | Colonnade tile only — already planned |
| **Sun room** | South infill court margin · glass after **KILN-C-UPGRADE** |
| **Greenhouse** | **Outside** east wall — not impluvium |
| **River culvert** | Upgrade from **SOIL-PIT-1** if needed |

---

## Materials · brick hoard *(domus + officina campus)*

*Domus alone is smaller than the old 2F T-plan — **total fired brick** stays **high** because **officina / molendinum / incudinarium** upgrades run in parallel.*

| Bucket | Est. fired brick | Notes |
|--------|------------------|-------|
| **Domus H-V3** | **~700–1,000** | Single-wythe mures · hypocaust east only · colonnade |
| **Officina + telarium** (W-1) | **~80–150** | LOOM-WING cheek · lintels · **MURE-R1** socle class |
| **Molendinum** (MILLHOUSE-3+) | **~40–80** | Forge-adjacent cheek · char sight · windbreak |
| **CHEM-LAB-WING-1** | **~200–280** | **5×6 m lab @ WW-YARD** · stone socle · before domus — [chem-lab-plan.md](chem-lab-plan.md) |
| **MILLHOUSE-2 mirror** *(later)* | **~120–160** | Replaces torn **3×4 m** Phase 1 · same rectangle grammar |
| **Incudinarium** (FORGE-D+) | **~60–120** | Bloomery lining · slag pad · **Tier 3** grog courses |
| **Kiln / terrace maintenance** | **~80–120** | B-series lining refresh · hypocaust spare |
| **Roof tile** *(domus)* | **~350–500** | Per-range sheds · porticus |
| **Floor tile** *(domus)* | **~120–200** | Culina · cubicula · compluvium margin |
| **Campus total (brick)** | **~1,250–1,750+** | **+lab wing ~200–280** · factory years @ quad press class |
| **Campus total (incl. tile)** | **~1,500–2,000+** | **piece-equivalent** hoard target before Stage 0 stake |

**Pipeline now:** **~52** stackable @ Kiln B · **~188 in CHEM-LAB walls** · **Stage 3 ~95%** ([chem-lab-plan.md](chem-lab-plan.md)) — **wood ~72 kg · clay ~31 kg · FS reserve @ pile 8.**

| Other | Est. |
|-------|------|
| **Cut stone** | High — socle · colonnade · impluvium |
| **Tier 3 lime** | **~60–100 kg** slaked equiv. (domus + forge beds) |
| **Oak** | Colonnade · bond beams · officina frames · **BARN timber stock** (straight reserve · WOOD-SORT-DOCTRINE) |

---

## Tech dependencies

| Rung | Need |
|------|------|
| **CERAMICS-FACTORY-2** | Brick + tile rhythm |
| **STONE-CUT-1** | Socle · colonnade |
| **Tier 3 lime** | Plaster · bedding · pit seal |
| **AQUEDUCT-1** ✓ | Kitchen · bath |
| **CRANE-1** ✓ | Tile · pillar caps |
| **FLUSH-TOILET-1** / latrine grammar | **DR-LT-01 ✓ d938 · east margin ✓ d1252** |
| **CHEM-LAB-1** | Glass addons later |

---

## Risks · maintenance *(revised plan)*

| Topic | Courtyard plan |
|-------|----------------|
| **Hardest ongoing work** | **Impluvium drain + bath hypocaust** — still real, but **quarantined** to east wing |
| **First likely failure** | **Impluvium clog** or **grease trap** — both **visible** from court |
| **Dropped risks** | 2F leak · stair · temp 2F roof · dual furnace · cavity rot · long blackwater culvert (if SOIL-PIT) |
| **Still watch** | Lime re-point · tile valley leaks · **SOIL-PIT** fill · earthquake bond beams |

**Maintenance cadence:** weekly **impluvium + porticus drain** in rain season · seasonal **hypocaust + flue** when heating · annual **colonade post** check.

---

## Geology · climate *(unchanged)*

| Topic | Note |
|-------|------|
| **Frost heave** | Low priority — **40–60 cm** drained footing enough |
| **Earthquake** | **Bond beams** · flexible colonnade pins · avoid brittle vaults until skilled |
| **Rain / clay** | **Impluvium** is the correct Roman answer — **central drain**, not fighting each wall |
| **Grade** | **Always S toward D-1** — protect H-v2 north cellar |

---

## Superseded plans *(archive)*

| Artifact | Status |
|----------|--------|
| **2F T-plan** | [house-diagram-labeled.png](images/house-diagram-labeled.png) — reference only |
| **H-v2 annex wings** | [hut-plans.png](images/hut-plans.png) — reference only |
| **MURE-2 double cavity** | Superseded by **MURE-R1** |

---

## Open decisions *(stake day)*

- [ ] **Impluvium** — shallow pool vs **drain only** *(✓ drain-only default filed d1099 · pool defer)*
- [x] **North tie** — **open colonnade passage ✓ d1246 · H-V3-NORTH-TIE-1 · SR covered option defer**
- [ ] **SOIL-PIT-1** exact margin site — fall · distance from well/aqueduct
- [ ] **Hypocaust** under cubicula — **defer yes/no**

---

*Update when a stage completes. Linked from [map.md](map.md).*
